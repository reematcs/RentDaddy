  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        ...
        #{ name = "POSTGRES_HOST", value = "main-postgres.rentdaddy.local" },
        { name = "NEXT_PRIVATE_DATABASE_URL", value = "postgresql://documenso:password@database:5432/documenso" },
        { name = "NEXT_PRIVATE_DIRECT_DATABASE_URL", value = "postgresql://documenso:password@database:5432/documenso" },
      ]
      secrets = [
...
resource "aws_ecs_task_definition" "documenso_postgres" {
  ...
  network_mode = "bridge" # Switch to bridge mode instead of awsvpc
